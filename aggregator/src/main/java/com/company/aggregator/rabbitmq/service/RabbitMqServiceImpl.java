...
import com.company.aggregator.websocket.dto.WebSocketSendMessageDto;
...
public class RabbitMqServiceImpl implements RabbitMqService {
    ...
    public void receiveVacanciesParser(List<com.company.aggregator.rabbitmq.dto.vacancy.ReceiveMessageDto> receiveMessageDtoList) {
        ...
        if (!receiveMessageDtoList.isEmpty()) {
            receiveMessageDtoList.removeIf(dto -> {
                try {
                    return vacancyService.findVacancyBySource(dto.getSource()) != null;
                } catch (VacancyNotFoundException e) {
                    return false;
                }
            });
            vacancyService.saveVacancies(receiveMessageDtoList, user);
        }
    ...
    public void receiveStatisticsParser(com.company.aggregator.rabbitmq.dto.statistics.ReceiveMessageDto message) {
        ...
        if (message.getUsername() != null) {
            ...
            statisticsService.deleteStatisticsByUser(user);
            statisticsService.saveStatistics(user, message);
        }
    }

    @Override
    public void sendToVacanciesParser(com.company.aggregator.rabbitmq.dto.vacancy.SendMessageDto sendMessageDto) {
