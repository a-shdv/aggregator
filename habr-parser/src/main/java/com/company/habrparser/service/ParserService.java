...
public class ParserService {
    ...
    public void parseWebPage() {
        ...
        Document doc = null;
        try {
            doc = Jsoup.connect("https://career.habr.com/vacancies/1000135854").get();
        } catch (IOException e) {
...
//        String[] locationAndEmploymentType = header.first().getElementsByClass("content-section").get(2).text()
//                .substring(31).split("•");
        Elements locationAndEmploymentType = header.first().getElementsByClass("content-section");

        Vacancy vacancy = Vacancy.builder()
                .title(header.first().getElementsByClass("page-title__title").text())
                .datePublished(header.first().getElementsByClass("basic-date").text())
                .salary(header.first().getElementsByClass("basic-salary basic-salary--appearance-vacancy-header").text())
                .company(header.first().getElementsByClass("link-comp link-comp--appearance-dark").last().text())

                .requirements(header.first().getElementsByClass("content-section").attr("h2", "Требования").first().text())
                .city(header.first().getElementsByClass("content-section").attr("h2", "Местоположение и тип занятости").first().text())
...
//        Document doc = null;
//        try {
//            doc = Jsoup.connect("https://career.habr.com/vacancies/1000133114").get();
//        } catch (IOException e) {
//            log.error(e.getMessage());
//        }
//        Elements header = doc.select("html body.vacancies_show_page div.page-container div.page-container__main div.page-width.page-width--responsive div.content-wrapper div.content-wrapper__main.content-wrapper__main--left div");
